  ...
  return (
    <div className="glass rounded-none p-5 border-2 border-white/5 hover:border-white/30 transition-all group relative overflow-hidden bg-arena-900">
      ...
      <div className="relative z-10 flex justify-between items-start mb-4">
        <div>
          <h3 className="text-xl font-black font-space tracking-tight text-white uppercase group-hover:text-emerald-400 transition-colors">{game.TurfName}</h3>
        ...
        {!isJoined ? (
          ...
          <>
            <button onClick={() => onChat(game.BookingID, game.HostUserID, game.HostName)}
              className="flex-1 px-5 py-3 border-2 border-indigo-500/50 bg-indigo-500/10 text-indigo-400 text-sm font-black font-space uppercase hover:bg-emerald-500 hover:border-emerald-500 hover:text-black transition-colors tracking-widest">
              SQUAD CHAT
            </button>
            <button onClick={() => onLeave(game.BookingID)}
              className="px-5 py-3 border-2 border-rose-500/50 bg-rose-500/10 text-rose-400 text-sm font-black font-space uppercase hover:bg-rose-500 hover:border-rose-500 hover:text-black transition-colors tracking-widest">
              LEAVE GAME
            </button>
          </>
        )}
